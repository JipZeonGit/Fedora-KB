...
- **游戏生态与优化**: 原生 RPM Steam 客户端、ProtonPlus 兼容层管理 和 Gamescope 微合成器（绕过 GNOME Mutter 修复 Proton 窗口标题栏返祖与渲染问题）。
...
*常用桌面应用、开发环境与 AI 工具链*
- [01-shell-install-apps.md](./04-Software-Installation/01-shell-install-apps.md) — 常用桌面应用指南 (MPV, VS Code, Edge, JetBrains Toolbox, 原生微信, WPS Office, WindTerm)
...
- [04-game-steam-setup.md](./04-Software-Installation/04-game-steam-setup.md) — Steam 游戏生态与 Gamescope 配置 (64-bit RT3 客户端、Gamescope 微合成器避坑、ProtonPlus 兼容层管理与 Proton-GE/DW-Proton 安装)
- [05-nodejs-setup.md](./04-Software-Installation/05-nodejs-setup.md) — Node.js 环境管理指南 (`fnm` 用户级管理、镜像加速与 Corepack)
- [06-ai-tools-and-cli.md](./04-Software-Installation/06-ai-tools-and-cli.md) — AI 编程工具与开发 CLI (OpenCode, MimoCode, agy, GitHub CLI, Qoder CN, TRAE CN)
- [07-podman-docker-setup.md](./04-Software-Installation/07-podman-docker-setup.md) — Podman 与 Docker Compose V2 容器配置 (Rootless 套接字、Compose V2 二进制、containers.conf 避坑与特权端口设置)
- [08-jdk-temurin-setup.md](./04-Software-Installation/08-jdk-temurin-setup.md) — Java / JDK 开发环境指南 (Adoptium 官方 RPM 源、Eclipse Temurin 21 / 17 / 8 多版本安装与 `alternatives` 无缝切换)
- [09-android-tools-adb-setup.md](./04-Software-Installation/09-android-tools-adb-setup.md) — Android Platform Tools 调试指南 (DNF 原生安装 `android-tools` 包含 `adb`/`fastboot`、常用命令与 `adbusers` / udev 权限调优)
...
- **文字与文档内容**: 采用 [CC-BY-4.0 (知识共享 署名 4.0 国际许可协议)](https://creativecommons.org/licenses/by/4.0/deed.zh) 开源。转载与二次创作请注明原作者与出处。*(注：部分摘录自第三方官方文档的文件如 [`05-nodejs-setup.md`](./04-Software-Installation/05-nodejs-setup.md) 和 [`02-flatpak-setup.md`](./04-Software-Installation/02-flatpak-setup.md) 版权归原官方/维护者所有，不适用于本 CC 许可)*
